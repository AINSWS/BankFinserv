...
class BankReconciliationUI:
    ...
    def _create_reconciliation_tab(self, notebook, recon_engine):
        ...
        try:
            ...
            # Extract summary from reconciliation result
    ...
    def _export_simple_results(self, results):
        ...
        try:
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                ...
                # Create comprehensive summary sheet with Phase 3 information
                self._create_comprehensive_summary_sheet(writer, summary, results)
    ...
    def _create_comprehensive_summary_sheet(self, writer, summary, results):
        """Create a comprehensive summary sheet with Phase 3 information"""
        try:
            # Get Phase 3 information from results
            phase3_result = results.get('phase3_result', {})
            phase3_analysis = phase3_result.get('phase3_analysis', {}) if phase3_result else {}
            
            # Basic reconciliation metrics
            basic_metrics = [
                ['BASIC RECONCILIATION METRICS', ''],
                ['Total Records', summary.get('total_unique_loans', 0)],
                ['Perfect Matches', summary.get('perfect_matches', 0)],
                ['Minor Matches', summary.get('minor_matches', 0)],
                ['Total Matches', summary.get('total_matches', 0)],
                ['Amount Mismatches', summary.get('amount_mismatches', 0)],
                ['Match Rate %', f"{summary.get('match_percentage', 0):.1f}%"],
                ['', ''],
            ]
            ...
            if phase3_analysis:
                phase3_metrics = [
                    ['PHASE 3 RECONCILIATION METRICS', ''],
                    ['Phase 3 Newly Matched', phase3_analysis.get('newly_matched_count', 0)],
                    ['Phase 3 Still Mismatched', phase3_analysis.get('still_mismatched_count', 0)],
                    ['Phase 3 Match Improvement', f"{phase3_analysis.get('match_improvement_percentage', 0):.1f}%"],
                    ['', ''],
                ]
            ...
            processing_stats = []
            if 'bank_ledger' in results:
                bank_info = results['bank_ledger']
                if isinstance(bank_info, dict) and bank_info.get('status') == 'success':
                    parsed_data = bank_info.get('parsed_data', {})
                    processing_stats = [
                        ['BANK LEDGER PROCESSING', ''],
                        ['Total Bank Entries', parsed_data.get('total_rows', 0)],
                        ['Valid Loan Entries', parsed_data.get('valid_loan_entries', 0)],
                        ['Filtered Out Entries', parsed_data.get('filtered_out_entries', 0)],
                        ['Processing Success Rate', f"{(parsed_data.get('valid_loan_entries', 0) / max(parsed_data.get('total_rows', 1), 1) * 100):.1f}%"],
                        ['', ''],
                    ]
            
            # Status breakdown from actual data
            status_breakdown = []
            main_data = results.get('simplified_report')
            if main_data is not None and not main_data.empty:
                # Find status column
                status_col = None
                for col in ['status', 'reconciliation_status']:
                ...
                if status_col:
                    status_counts = main_data[status_col].value_counts()
                    status_breakdown = [['STATUS BREAKDOWN', '']]
                    for status, count in status_counts.items():
                        status_breakdown.append([str(status), count])
                    status_breakdown.append(['', ''])
            
            # Combine all metrics
            all_metrics = basic_metrics + phase3_metrics + amount_metrics + processing_stats + status_breakdown
            
            # Add timestamp
            all_metrics.extend([
                ['EXPORT INFORMATION', ''],
                ['Export Date', pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')],
                ['Report Type', 'Complete Reconciliation with Phase 3 Analysis']
            ])
            
            # Create DataFrame and export
            summary_df = pd.DataFrame(all_metrics, columns=['Metric', 'Value'])
            summary_df.to_excel(writer, sheet_name='Summary', index=False)
            print(f"   ✓ Comprehensive summary created with Phase 3 metrics")
            
        except Exception as e:
            print(f"   ⚠️ Error creating comprehensive summary: {e}")
            # Fallback to basic summary
            basic_summary_data = {
                'Metric': ['Total Records', 'Total Matches', 'Amount Mismatches', 'Match Rate %'],
                'Value': [
            ...
            basic_summary_df = pd.DataFrame(basic_summary_data)
            ...
            for sheet_name in wb.sheetnames:
                ...
                try:
                    ...
                    if 'Matched' in sheet_name and max_row and max_col and max_row > 1:
                        ...
                        # Color only duplicate group IDs in red
                        group_id_col = None
                        # Look for group ID column (try common variations)
                        for col in range(1, max_col + 1):
                            header_value = ws.cell(row=1, column=col).value
                            if header_value and any(keyword in str(header_value).lower() for keyword in ['group', 'group_name', 'group_id']):
                                group_id_col = col
                                print(f"   🔍 Found group column at {col}: {header_value}")
                                break
                        ...
                        if group_id_col:
                            ...
                            # First pass: count group IDs
                            for row in range(2, row_limit):
                                group_value = ws.cell(row=row, column=group_id_col).value
                                if group_value:
                                    group_key = str(group_value).strip()
                                    group_counts[group_key] = group_counts.get(group_key, 0) + 1
                            
                            # Second pass: color duplicate group IDs
                            duplicate_count = 0
                            for row in range(2, row_limit):
                                group_value = ws.cell(row=row, column=group_id_col).value
                                if group_value:
                                    group_key = str(group_value).strip()
                                    if group_counts[group_key] >= 2:  # Duplicate found
                                        for col in range(1, max_col + 1):
                                            ws.cell(row=row, column=col).fill = mismatch_fill
                                        duplicate_count += 1
                            
                            print(f"   🔴 Colored {duplicate_count} duplicate group ID entries in red")
                            print(f"   📊 Found {len([k for k, v in group_counts.items() if v >= 2])} unique duplicate group IDs")
                        else:
                            print(f"   ⚠️ No group column found - using default red coloring")
                            # Fallback to original behavior if no group column found
                            row_limit = min(max_row + 1, 1000)
                            for row in range(2, row_limit):
                                for col in range(1, max_col + 1):
                                    ws.cell(row=row, column=col).fill = mismatch_fill
