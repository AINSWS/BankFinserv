...
# Use relative imports for better packaging compatibility
...
class ModularReconciliationEngine:
    ...
    def merge_pivot_tables_comparison(self, include_stage2=True, include_phase3=True):
        ...
        # Use MergeOperationsHandler to merge the pivot tables
    ...
    def _apply_stage2_to_main_data(self, reconciliation_result):
        ...
        try:
            ...
            if stage2_result.get('status') != 'success':
                ...
                print("⚠️ Stage 2 processing failed, using original reconciliation data")
                return reconciliation_result
            
            newly_matched = stage2_result.get('newly_matched', pd.DataFrame())
            
            if newly_matched.empty:
                # No Stage 2 matches found, return original data
                return reconciliation_result
            
            print(f"🔄 Applying {len(newly_matched)} Stage 2 results to main reconciliation data...")
            ...
            for _, stage2_record in newly_matched.iterrows():
                ...
                if not matching_rows.empty:
                    # Update the record with Stage 2 results
                    ...
                    if 'reconciliation_method' in stage2_record:
                        ...
                        updated_main_data.loc[idx, 'reconciliation_method'] = 'Stage 2: Group Payment'
